...
def train_sbert(run_path, df_train, df_val, df_test, answer_column, target_column, id_column, base_model, batch_size, num_epochs, save_model=False, num_val_pairs_per_example=None, patience=None):
    ...
    transformer = models.Transformer(base_model, max_seq_length=128)
    ...
    if num_val_pairs_per_example is not None:
        ...
        val_examples = val_examples.sample(num_val_pairs_per_example, random_state=random_state)
    ...
    # Tune the model
    model.fit(train_objectives=[(train_dataloader, train_loss)], epochs=num_epochs, warmup_steps=0, evaluator=evaluator, output_path=model_path, save_best_model=True, show_progress_bar=True, evaluation_steps=num_batches_per_round)
    ...
    gold, max_pred, avg_pred, _ = eval_sbert(run_path=run_path, df_test=df_test, df_ref=df_ref, id_column=id_column, answer_column=answer_column, target_column=target_column)
